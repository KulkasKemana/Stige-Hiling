...
let isCurrentA = true; // untuk swap peran img

function setCarousel(idx, isAuto = false) {
  if(idx === currentIdx) return;
  const imgA = document.getElementById('carousel-img-current');
  const imgB = document.getElementById('carousel-img-next');
  ...
  imgIn.style.zIndex = 0;
  imgOut.style.zIndex = 0;
  ...
  setTimeout(() => {
    ...
    imgOut.style.transform = "translateX(100%)";
    document.getElementById('carousel-caption').innerText = images[idx].caption;
    currentIdx = idx;
    updateDots(idx);
    isCurrentA = !isCurrentA; // swap peran
  }, 710);
}
